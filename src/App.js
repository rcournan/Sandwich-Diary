...
const AVATARS = ["🥪","🍞","🥓","🧀","🥬","🍅","🫙","🥒","🌶","🧅","🥩","🫒"];
const SENTIMENTS = ["🤩","😋","😌","🥹","🔥","😐","😬","💀"];
const BREAD_TYPES = ["Sourdough","Ciabatta","Baguette","Focaccia","Brioche","Rye","Cuban","Hoagie","Sub/Hero","Pita","Bagel","English Muffin","Pretzel","Flatbread","Pumpernickel","Multigrain","Potato Roll","Slider Bun","Tortilla Wrap","Gluten-Free","Croissant","Other"];

// ── Photo Carousel ────────────────────────────────────────────────────────────
function PhotoCarousel({ photos, name }) {
  const [idx, setIdx] = useState(0);
  if (!photos?.length) return <div className="card-photo-placeholder">🥪</div>;
  return (
    <div className="photo-carousel">
      <div className="photo-carousel-track" style={{ transform:`translateX(-${idx*100}%)` }}>
        {photos.map((p,i) => <img key={i} src={p} className="card-photo" alt={`${name} ${i+1}`} />)}
      </div>
      {photos.length > 1 && (<>
        {idx > 0 && <button className="photo-nav prev" onClick={() => setIdx(i=>i-1)}>‹</button>}
        {idx < photos.length-1 && <button className="photo-nav next" onClick={() => setIdx(i=>i+1)}>›</button>}
        <div className="photo-dot-row">
          {photos.map((_,i) => <button key={i} className={`photo-dot${i===idx?" active":""}`} onClick={() => setIdx(i)} />)}
        </div>
      </>)}
    </div>
  );
...
function SandwichMap({ entries, currentUser }) {
  ...
  const visible = entries.filter(e => e.coords && (filter==="all" || (filter==="mine" ? e.user===currentUser : e.user!==currentUser)));
  ...
  useEffect(() => {
    ...
    s.onload = () => { const hs=document.createElement("script"); hs.src="https://cdnjs.cloudflare.com/ajax/libs/leaflet.heat/0.2.0/leaflet-heat.js"; hs.onload=initMap; document.head.appendChild(hs); };
    document.head.appendChild(s);
    function initMap() {
      const L=window.L, map=L.map(mapRef.current,{zoomControl:true}).setView([42.36,-71.06],11);
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap",maxZoom:19}).addTo(map);
      mapInstanceRef.current=map; renderMarkers();
    }
  }, []);
...
function AuthScreen() {
  ...
  const IS = {width:"100%",fontFamily:"'Courier Prime',monospace",fontSize:13,padding:"8px 10px",border:"2px solid #7DD8D0",borderRadius:2,background:"white",color:"#1A2744"};
  const LS = {fontFamily:"'Special Elite',serif",fontSize:10,letterSpacing:2,textTransform:"uppercase",color:"#1A2744",display:"block",marginBottom:4};
  const BTN = {width:"100%",fontFamily:"'Alfa Slab One',serif",fontSize:15,letterSpacing:2,padding:13,background:loading?"#ccc":"#F07828",color:"#1A2744",border:"3px solid #1A2744",borderRadius:2,cursor:loading?"wait":"pointer",textTransform:"uppercase",boxShadow:"3px 3px 0 #1A2744",marginTop:4};
  ...
  async function doSignup() {
    ...
    try {
      ...
      // Check invite if not first
      if (!isFirst) {
        const { data:inv } = await supabase.from("invites").select("*").eq("code",invite.toUpperCase()).eq("used",false).maybeSingle();
      ...
      // Insert profile row (keyed by email, not auth id)
      ...
      if (profErr) { setLoading(false); setError("Profile error: " + profErr.message); return; }
      ...
      // If email confirm is OFF, session arrives now via onAuthStateChange — done
    ...
    <div style={{minHeight:"100vh",background:"#1A2744",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,overflowY:"auto"}}>
      ...
      <div style={{background:"#FFFDF8",border:"2px solid #7DD8D0",borderRadius:4,padding:24,width:"100%",maxWidth:360}}>
        ...
        {mode==="reset" && (<>
          ...
          <div style={{marginBottom:12}}><label style={LS}>Email</label><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" style={IS}/></div>
          <button onClick={doReset} style={BTN} disabled={loading}>{loading?"Sending…":"Send Reset Link"}</button>
        ...
        {(mode==="login"||mode==="signup") && (<>
          ...
          {mode==="signup" && <div style={{marginBottom:12}}><label style={LS}>Username</label><input value={username} onChange={e=>setUsername(e.target.value)} placeholder="e.g. SandwichSally" style={IS}/></div>}

          <div style={{marginBottom:12}}><label style={LS}>Email</label>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" style={IS} onKeyDown={e=>e.key==="Enter"&&mode==="login"&&doLogin()}/>
          </div>

          <div style={{marginBottom:mode==="login"?6:12}}><label style={LS}>Password</label>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" style={IS} onKeyDown={e=>e.key==="Enter"&&mode==="login"&&doLogin()}/>
          </div>
          ...
          {mode==="signup" && (<>
            <div style={{marginBottom:12}}>
              ...
              <input value={invite} onChange={e=>setInvite(e.target.value)} placeholder="e.g. SANDWICH" style={{...IS,textTransform:"uppercase",letterSpacing:2}}/>
            ...
            <div style={{marginBottom:16}}>
              <label style={LS}>Pick your emoji</label>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {AVATARS.map(a=><span key={a} onClick={()=>setEmoji(a)} style={{fontSize:22,cursor:"pointer",padding:4,border:`2px solid ${emoji===a?"#F07828":"transparent"}`,borderRadius:4,background:emoji===a?"#FEF0E6":"transparent"}}>{a}</span>)}
              </div>
            </div>
          </>)}

          <button onClick={mode==="login"?doLogin:doSignup} style={BTN} disabled={loading}>
...
const styles = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  :root{--orange:#F07828;--orange-dark:#C85E10;--orange-pale:#FEF0E6;--navy:#1A2744;--mint:#7DD8D0;--mint-dark:#3AADA4;--mint-pale:#E3F7F5;--cream:#FFFDF8}
  ...
  .photo-dot{width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,0.5);border:none;cursor:pointer;padding:0}
  ...
  .loading-screen{min-height:100vh;background:#1A2744;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px}
  .loading-title{font-family:'Alfa Slab One',serif;font-size:28px;color:#F07828;letter-spacing:1px}
  .loading-error{font-family:'Courier Prime',monospace;font-size:13px;color:#F07828;background:rgba(0,0,0,0.3);padding:12px 16px;border-radius:4px;max-width:300px;text-align:center;line-height:1.5}
...
function normalizeEntry(e) {
  return {
    ...
    mainIngredient: e.main_ingredient || "",
...
export default function App() {
  ...
  const [form, setForm] = useState({
    sandwichName:"", location:"", coords:null, breadType:"Sourdough", customBread:"",
    homemade:false, price:"", rating:5, sentiment:"😋",
    wouldOrderAgain:true, notes:"", ingredients:[], mainIngredient:"", photos:[], newIngredient:"",
  });
  ...
  async function loadProfile(userEmail) {
    ...
    try {
      ...
      if (error) { setLoadError("DB error: " + error.message); setLoading(false); return; }
      if (!data) { setLoadError("No profile found for " + userEmail + ". Try signing up."); setLoading(false); return; }
  ...
  async function loadEntries() {
    const { data } = await supabase
      .from("entries")
      .select("*, likes(username), comments(id,username,text,created_at)")
  ...
  async function submitComment(entryId) {
    ...
    const { data } = await supabase.from("comments")
      .insert({ entry_id: entryId, username: profile.username, text })
      .select().single();
    if (data) {
      setEntries(prev => prev.map(e => e.id !== entryId ? e : {
  ...
  function handlePhoto(e) {
    Array.from(e.target.files || []).forEach(file => {
      setForm(f => {
        ...
        r.onload = ev => setForm(f2 => ({ ...f2, photos: f2.photos.length < 5 ? [...f2.photos, ev.target.result] : f2.photos }));
        r.readAsDataURL(file); return f;
      });
    }); e.target.value = "";
  }
  ...
  function grabGPS() {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      ...
      try {
        ...
        setForm(f => ({ ...f, location: addr, coords: [lat,lon] })); setGpsStatus("✓ Location set");
      } catch {
        setForm(f => ({ ...f, location: `${lat.toFixed(4)}, ${lon.toFixed(4)}`, coords: [lat,lon] })); setGpsStatus("✓ Coords saved");
      }
  ...
  async function submitEntry() {
    ...
    setSaving(true);
    const breadType = form.breadType === "Other" && form.customBread.trim() ? form.customBread.trim() : form.breadType;
    ...
    setForm({ sandwichName:"", location:"", coords:null, breadType:"Sourdough", customBread:"", homemade:false, price:"", rating:5, sentiment:"😋", wouldOrderAgain:true, notes:"", ingredients:[], mainIngredient:"", photos:[], newIngredient:"" });
    setShowModal(false); setTab("feed");
  }
      ...
      <div key={entry.id} className="card">
        ...
        <div className="card-body">
          ...
          {entry.ingredients?.length > 0 && (
            <div className="ingredients">
              {entry.ingredients.map((ing,i) => (
                <span key={i}>
                  {i > 0 && " · "}
                  {ing === entry.mainIngredient
                    ? <strong style={{color:"var(--orange)",fontWeight:"bold"}}>★ {ing}</strong>
                    : ing}
                </span>
              ))}
            </div>
          ...
          <div className="card-actions">
            <button className={`action-btn${liked ? " liked" : ""}`} onClick={() => toggleLike(entry.id)}>{liked ? "♥" : "♡"} {entry.likes.length}</button>
        ...
        <div className="comments-section">
          ...
          <div className="comment-input-row">
            <input className="comment-input" placeholder="Add a comment…" value={commentInputs[entry.id]||""}
  ...
  if (loading) return (
    <div className="loading-screen">
      <div className="loading-title">Stacked ★</div>
      {loadError && (<>
        <div className="loading-error">{loadError}</div>
        <button onClick={() => supabase.auth.signOut().then(() => { setLoadError(""); setLoading(false); })}
          style={{fontFamily:"'Special Elite',serif",fontSize:11,padding:"8px 16px",background:"#F07828",color:"#1A2744",border:"none",borderRadius:2,cursor:"pointer",letterSpacing:1,textTransform:"uppercase"}}>
          Sign out &amp; try again
        </button>
      </>)}
    </div>
      ...
      <div className="app">
        ...
        {tab==="people" && (<>
          ...
          <div className="people-list">
            {allUsers.length===0
              ? <div className="empty-state"><span className="big">🥪</span>No other sandwich artists yet!</div>
              : (() => {
                  const filtered = allUsers.filter(u => u.username.toLowerCase().includes(peopleSearch.toLowerCase()));
                  if (!filtered.length) return <div className="empty-state"><span className="big">🔍</span>No one matching "{peopleSearch}"</div>;
        ...
        {tab==="profile" && (<>
          <div className="profile-card">
            ...
            <div className="profile-stats">
              ...
              <div className="stat"><span className="stat-num">{myEntries.length?(myEntries.reduce((a,e)=>a+e.rating,0)/myEntries.length).toFixed(1):"—"}</span><span className="stat-label">Avg ★</span></div>
            </div>
          </div>
          ...
          <div className="invite-box">
            ...
            {inviteCodes.length===0
              ? <div style={{fontSize:12,color:"#aaa",fontStyle:"italic",marginBottom:8}}>No codes left — generate one below</div>
        ...
        {showModal && (
          <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setShowModal(false)}>
            <div className="modal">
              ...
              <div className="form-group"><label className="form-label">Sandwich Name *</label>
                <input className="form-input" placeholder="e.g. The Italian Stallion" value={form.sandwichName} onChange={e=>setForm(f=>({...f,sandwichName:e.target.value}))} /></div>

              <div className="form-group"><label className="form-label">Location</label>
                <div className="location-row">
                  <input className="form-input" placeholder="e.g. Mike's Deli, Boston" value={form.location} onChange={e=>setForm(f=>({...f,location:e.target.value}))} />
              ...
              <div className="form-group"><label className="form-label">Photos ({form.photos.length}/5)</label>
                <input type="file" accept="image/*" multiple ref={fileRef} style={{display:"none"}} onChange={handlePhoto} />
                {form.photos.length>0 && <div className="photo-grid">{form.photos.map((p,i)=><div key={i} className="photo-thumb"><img src={p} alt={`photo ${i+1}`}/><button className="photo-thumb-remove" onClick={()=>removePhoto(i)}>×</button></div>)}</div>}
                <div className={`photo-upload${form.photos.length>=5?" full":""}`} onClick={()=>form.photos.length<5&&fileRef.current.click()}>
              ...
              <div className="form-group"><label className="form-label">Ingredients</label>
                <div className="ingredients-input-row">
                  <input className="form-input" placeholder="e.g. Mortadella" value={form.newIngredient} onChange={e=>setForm(f=>({...f,newIngredient:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addIngredient()} />
                  <button className="comment-submit" style={{whiteSpace:"nowrap",borderRadius:2}} onClick={addIngredient}>Add</button>
                </div>
                {form.ingredients.length>0 && (
                  <div style={{marginTop:6}}>
                    <div style={{fontFamily:"'Special Elite',serif",fontSize:9,letterSpacing:2,textTransform:"uppercase",color:"#3AADA4",marginBottom:5}}>★ Tap an ingredient to mark it as the star</div>
                    <div>
                      {form.ingredients.map((ing,i) => (
                        <span key={i} className="ingredient-tag" style={{cursor:"pointer",borderColor:form.mainIngredient===ing?"var(--orange)":"var(--mint-dark)",background:form.mainIngredient===ing?"var(--orange-pale)":"var(--mint-pale)"}}
                          onClick={()=>setForm(f=>({...f,mainIngredient:f.mainIngredient===ing?"":ing}))}>
                          {form.mainIngredient===ing && <span style={{color:"var(--orange)",marginRight:2}}>★</span>}
                          {ing}
                          <button className="ingredient-remove" onClick={ev=>{ev.stopPropagation();removeIngredient(i)}}>×</button>
                        </span>
                      ))}
                    </div>
                  </div>
              ...
              <div className="form-group"><label className="form-label">Bread Type</label>
                <select className="form-select" value={form.breadType} onChange={e=>setForm(f=>({...f,breadType:e.target.value,customBread:""}))}>
                  {BREAD_TYPES.map(b=><option key={b}>{b}</option>)}
                </select>
                {form.breadType==="Other" && (
                  <input className="form-input" style={{marginTop:8}} placeholder="Describe the bread…" value={form.customBread} onChange={e=>setForm(f=>({...f,customBread:e.target.value}))} />
                )}
              </div>

              <div className="form-group"><label className="form-label">Rating</label>
                <div className="star-rating">{[1,2,3,4,5].map(n=><span key={n} className={`star-opt${n<=form.rating?" filled":""}`} onClick={()=>setForm(f=>({...f,rating:n}))}>★</span>)}</div>
              </div>

              <div className="form-group"><label className="form-label">Vibe</label>
                <div className="sentiment-picker">{SENTIMENTS.map(s=><span key={s} className={`sentiment-opt${form.sentiment===s?" selected":""}`} onClick={()=>setForm(f=>({...f,sentiment:s}))}>{s}</span>)}</div>
              </div>

              <div className="form-group">
                <div className="toggle-row"><button className={`toggle${form.homemade?" on":""}`} onClick={()=>setForm(f=>({...f,homemade:!f.homemade}))}/><span className="toggle-label">🏠 Homemade</span></div>
                <div className="toggle-row"><button className={`toggle${form.wouldOrderAgain?" on":""}`} onClick={()=>setForm(f=>({...f,wouldOrderAgain:!f.wouldOrderAgain}))}/><span className="toggle-label">↩ Would eat again</span></div>
              </div>

              <div className="form-group"><label className="form-label">Price</label>
                <input className="form-input" placeholder="e.g. $12" value={form.price} onChange={e=>setForm(f=>({...f,price:e.target.value}))} /></div>

              <div className="form-group"><label className="form-label">Notes</label>
                <textarea className="form-textarea" placeholder="What made it special? What would you change?" value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} /></div>

              <button className="submit-btn" onClick={submitEntry} disabled={saving}>{saving?"Saving…":"Log This Sandwich"}</button>
